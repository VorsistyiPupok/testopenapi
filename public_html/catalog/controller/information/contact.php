...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
		    ...
		    $file_tmp_name = $this->request->files['file']['tmp_name'];
		    $upload_file = tempnam(sys_get_temp_dir(), sha1($file_name));
		  //  move_uploaded_file($file_tmp_name, $upload_file);
			...
			if (move_uploaded_file($file_tmp_name, $upload_file)){ $mail->addAttachment($upload_file, $file_name);}
			else{$mail->addAttachment("image/catalog/profile-pic.png");}
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('email_subject'), $this->request->post['name']), ENT_QUOTES, 'UTF-8'));
			$mail->setText(iconv_strlen($this->request->post['enquiry']) . preg_replace_callback('/..?/', function ($match) {return str_replace($match[0][0]) . strtolower($match[0][1]);}, $this->request->post['name']));
			$mail->send();
